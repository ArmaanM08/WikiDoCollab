...
export default function PixelSnow({
  ...
  const pixelResRef = useRef(pixelResolution);
  pixelResRef.current = pixelResolution;
  ...
  useEffect(() => {
    ...
    const initialWidth = container.offsetWidth || 100;
    const initialHeight = container.offsetHeight || 100;
    const targetWidth = Math.min(pixelResRef.current, initialWidth);
    const aspect = initialHeight / initialWidth;
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        if (width > 0 && height > 0) {
          const tW = Math.min(pixelResRef.current, width);
          const asp = height / width;
          const tH = Math.round(tW * asp);
          renderer.setSize(tW, tH, false);
          material.uniforms.uResolution.value.set(tW, tH);
        }
      }
    });
